...
angular.module('exam')
    .component('exam', {
        templateUrl: 'components/exam/exam.template.html',
        controller: function ExamController($scope, $http, $state, $injector, $stateParams, localStorageService, $modal, $timeout, ErrorService) {
            ...
            var startModal = $modal({
               scope: $scope,
                templateUrl: 'components/exam/startModal.template.html',
                show: false
            });
            ...
            var endModal = $modal({
                scope: $scope,
                templateUrl: 'components/exam/endModal.template.html',
                show: false
            });
            ...
            var successModal = $modal({
                scope: $scope,
                templateUrl: 'components/exam/successModal.template.html',
                show: false
            });
            ...
            var failModal = $modal({
                scope: $scope,
                templateUrl: 'components/exam/failModal.template.html',
                show: false
            });
            ...
            $scope.submit = function () {
                ...
                $http.post('/exam/v1/exam/submit/official/' + localStorageService.get("user").username + "?answerPaperId=" + $scope.answerPapers.id, $scope.paper).then(function (success) {
                    //显示modal
                    successModal.$promise.then(successModal.show);
                    //隐藏modal
                    $timeout(function () {
                        successModal.$promise.then(successModal.hide);
                        $injector.get('$state').go('recently');
                    }, 1000);
                }, function (error) {
                    //显示modal
